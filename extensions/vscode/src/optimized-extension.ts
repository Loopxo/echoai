...
export function activate(context: vscode.ExtensionContext) {
    ...
    try {
        ...
        const resourceManager = OptimizedResourceManager.getInstance(new ConfigurationManager());
        performanceReporter = new PerformanceReporter(resourceManager);
        ...
        vscode.window.showInformationMessage(
            'Echo AI Optimized - Core features ready, advanced features load on demand',
            'Open CLI',
            'Performance Report'
        ).then(selection => {
...
function registerEssentialCommands(context: vscode.ExtensionContext): void {
    ...
    const showCLICommand = vscode.commands.registerCommand('echo-ai.showCLI', async () => {
        if (!cliService) {
            ...
            cliService = MinimalCLIService.getInstance(context, echoProvider, new ConfigurationManager());
        }
        cliService.showCLI();
    });
...
function updateStatusBar(status: 'ready' | 'loading' | 'error'): void {
