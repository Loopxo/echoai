...
import { SecurityRemediationEngine, RemediationPlan } from './SecurityRemediationEngine';
...
export interface SecurityMetrics {
    ...
    trendData: {
        ...
        severityTrends: Array<{date: string, severity: string, count: number}>;
        fixedOverTime: Array<{date: string, fixed: number}>;
    };
    remediationProgress: {
        planned: number;
        inProgress: number;
...
export class SecurityDashboard {
    ...
    private securityDiagnostics: vscode.DiagnosticCollection;
    
    // Webview panels
    private dashboardPanel?: vscode.WebviewPanel;
    private vulnerabilityPanel?: vscode.WebviewPanel;

    constructor(
        context: vscode.ExtensionContext,
        echoProvider: EchoAIProvider,
        vulnerabilityScanner: VulnerabilityScanner,
        remediationEngine: SecurityRemediationEngine
    ) {
    ...
    async showSecurityDashboard(): Promise<void> {
        ...
        this.dashboardPanel = vscode.window.createWebviewPanel(
            'echo-ai-security-dashboard',
            '🛡️ Echo AI Security Dashboard',
            ...
            if (latestScan) {
                ...
                if (criticalCount > 0 || highCount > 0) {
                    ...
                    vscode.window.showInformationMessage(
                        `Security scan completed. Found ${latestScan.summary.total} issues total.`
                    );
                }
            }
    ...
    async startAutomatedRemediation(): Promise<void> {
        const latestScan = this.vulnerabilityScanner.getLatestScanResult();
        ...
        try {
            ...
            const action = await vscode.window.showInformationMessage(
                `Created remediation plan with ${plan.actions.length} actions. Estimated time: ${Math.round(plan.estimatedTime / 60)} minutes.`,
                'Execute Plan',
                'View Plan',
                'Cancel'
            );
    ...
    async executeRemediationPlan(planId: string): Promise<void> {
        ...
        try {
            await vscode.window.withProgress({
                ...
                if (successful > 0) {
                    vscode.window.showInformationMessage(
                        `Remediation completed: ${successful} actions succeeded, ${failed} failed`
                    );
    ...
    async generateComplianceReport(framework: ComplianceReport['framework']): Promise<ComplianceReport> {
        const latestScan = this.vulnerabilityScanner.getLatestScanResult();
    ...
    private async getDashboardWebviewContent(): Promise<string> {
...
<body>
    ...
    private updateMetricsFromScan(scanResult: SecurityScanResult): void {
        this.metrics = {
    ...
    private addScanToHistory(scanResult: SecurityScanResult): void {
    ...
    private generateAlertsFromScan(scanResult: SecurityScanResult): void {
    ...
    private updateSecurityDiagnostics(vulnerabilities: SecurityVulnerability[]): void {
        this.securityDiagnostics.clear();
        
        // Group vulnerabilities by file
        const vulnerabilitiesByFile = new Map<string, SecurityVulnerability[]>();
    ...
    private createDiagnosticFromVulnerability(vulnerability: SecurityVulnerability): vscode.Diagnostic {
        ...
        const diagnostic = new vscode.Diagnostic(
            range,
            `[${vulnerability.severity.toUpperCase()}] ${vulnerability.title}: ${vulnerability.description}`,
    ...
    private getScoreClass(riskScore: number): string {
        const securityScore = 100 - riskScore;
        if (securityScore >= 90) return 'score-excellent';
        if (securityScore >= 70) return 'score-good';
        if (securityScore >= 50) return 'score-warning';
        return 'score-critical';
    }

    private getScoreLabel(securityScore: number): string {
    ...
    private calculateCategoryDistribution(vulnerabilities: SecurityVulnerability[]): Map<string, number> {
    ...
    private calculateComplianceScore(complianceStatus: any): number {
        const frameworks = Object.values(complianceStatus) as boolean[];
        const compliantCount = frameworks.filter(status => status).length;
        return Math.round((compliantCount / frameworks.length) * 100);
    }

    private updateTrendData(scanResult: SecurityScanResult): any {
    ...
    private updateStatusBar(scanResult?: SecurityScanResult): void {
        if (scanResult) {
    ...
    private async generateOWASPComplianceReport(vulnerabilities: SecurityVulnerability[]): Promise<ComplianceReport> {
    ...
    private async generatePCIComplianceReport(vulnerabilities: SecurityVulnerability[]): Promise<ComplianceReport> {
    ...
    private async generateGDPRComplianceReport(vulnerabilities: SecurityVulnerability[]): Promise<ComplianceReport> {
    ...
    private async generateHIPAAComplianceReport(vulnerabilities: SecurityVulnerability[]): Promise<ComplianceReport> {
    ...
    private async generateSOC2ComplianceReport(vulnerabilities: SecurityVulnerability[]): Promise<ComplianceReport> {
    ...
    private async generateAndShowComplianceReport(): Promise<void> {
        const framework = await vscode.window.showQuickPick(
            ['OWASP', 'PCI-DSS', 'GDPR', 'HIPAA', 'SOC2'],
            { placeHolder: 'Select compliance framework' }
        );

        if (framework) {
            const report = await this.generateComplianceReport(framework as ComplianceReport['framework']);
            // Show compliance report in webview
        }
    }

    private async showVulnerabilitiesView(): Promise<void> {
        // Implementation for detailed vulnerabilities view
    }

    private async showRemediationPlan(plan: RemediationPlan): Promise<void> {
