...
import { CodebaseIndexer, IndexedFile, SymbolInfo } from './CodebaseIndexer';
...
export class IncrementalAnalyzer {
    ...
    private advancedDiagnostics: AdvancedDiagnosticsProvider;
    private analysisQueue: AnalysisTask[] = [];
    private isProcessing: boolean = false;
    private dependencyGraph: Map<string, DependencyGraph> = new Map();
    private analysisCache: Map<string, { result: AnalysisResult, expiry: number }> = new Map();
    ...
    private readonly DEPENDENCY_DEPTH = 5;
    ...
    constructor(codebaseIndexer: CodebaseIndexer, echoProvider: EchoAIProvider) {
        ...
        this.advancedDiagnostics = new AdvancedDiagnosticsProvider(echoProvider);
    ...
    async analyzeFile(
        ...
        analysisTypes: string[] = ['syntax', 'semantic', 'security', 'performance']
    ): Promise<void> {
        ...
        vscode.window.withProgress({
            ...
            for (let i = 0; i < files.length; i += this.BATCH_SIZE) {
                ...
                const promises = batch.map(file => 
                    this.analyzeFile(file.uri, this.calculatePriority(file), this.selectAnalysisTypes(file))
                );
            ...
            vscode.window.showInformationMessage(
                `Incremental analysis complete! Processed ${processed} files with ${this.performanceProfile.cacheHitRate.toFixed(2)} cache hit rate.`
            );
        });
    }

    private async prioritizeFiles(): Promise<IndexedFile[]> {
        const stats = this.codebaseIndexer.getStats();
        ...
        const remainingFiles: IndexedFile[] = [];
    ...
    private calculateFileScore(file: IndexedFile): number {
        let score = 0;
        ...
        score += (langPriority as any)[file.languageId] || 20;
    ...
    private calculatePriority(file: IndexedFile): AnalysisTask['priority'] {
    ...
    private selectAnalysisTypes(file: IndexedFile): string[] {
    ...
    private async buildDependencyGraph(uri: vscode.Uri): Promise<void> {
        const fileKey = uri.toString();
    ...
    private resolveImportPath(importPath: string, fromUri: vscode.Uri): string | null {
        ...
        if (importPath.startsWith('./') || importPath.startsWith('../')) {
            ...
            const resolvedPath = vscode.Uri.file(
                require('path').resolve(fromDir, importPath + '.ts')
            ).toString();
            return resolvedPath;
        }
    ...
    private findDependents(fileKey: string): string[] {
    ...
    private calculateAnalysisOrder(dependencies: string[], dependents: string[]): number {
    ...
    private enqueueTask(task: AnalysisTask): void {
        ...
        const insertIndex = this.analysisQueue.findIndex(t => 
            this.getTaskPriority(task) > this.getTaskPriority(t)
        );
    ...
    private getTaskPriority(task: AnalysisTask): number {
    ...
    private async processTask(task: AnalysisTask): Promise<void> {
        ...
        try {
            ...
            const timeoutPromise = new Promise<vscode.Diagnostic[]>((_, reject) => 
    ...
    private calculateConfidence(diagnostics: vscode.Diagnostic[], analysisTime: number): number {
        let confidence = 100;
    ...
    private async analyzeAffectedFiles(changedFile: string): Promise<void> {
    ...
    private cacheResult(fileKey: string, result: AnalysisResult): void {
    ...
    private getCachedResult(fileKey: string): AnalysisResult | null {
    ...
    private needsReanalysis(uri: vscode.Uri, previousResult: AnalysisResult): boolean {
        const indexed = this.codebaseIndexer.getFileIndex(uri);
    ...
    private getDependenciesFromGraph(fileKey: string): string[] {
    ...
    private estimateAnalysisTime(uri: vscode.Uri, analysisTypes: string[]): number {
        const indexed = this.codebaseIndexer.getFileIndex(uri);
        if (!indexed) return this.performanceProfile.averageAnalysisTime;
    ...
    private updatePerformanceMetrics(analysisTime: number, cacheHit: boolean, success: boolean): void {
    ...
    public getQueueStatus(): { size: number; processing: boolean; activeCount: number } {
        return {
