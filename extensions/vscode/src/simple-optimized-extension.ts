...
export async function activate(context: vscode.ExtensionContext) {
    ...
    try {
        ...
        vscode.window.showInformationMessage(
            'Echo AI Optimized - Reduced memory footprint, features load on demand',
            'Show Performance'
        ).then(selection => {
...
async function initializeCoreProviders(): Promise<void> {
    try {
        ...
        if (memUsage < 300) {
            ...
            if (memUsage < 200 && configManager.get<boolean>('enableInlineCompletion', true)) {
...
function registerOptimizedCommands(context: vscode.ExtensionContext): void {
...
function showPerformanceInfo(): void {
    const memUsage = process.memoryUsage();
