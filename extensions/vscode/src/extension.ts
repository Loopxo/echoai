...
export function activate(context: vscode.ExtensionContext) {
    ...
    const showCodebaseStatsCommand = vscode.commands.registerCommand('echo-ai.showCodebaseStats', async () => {
        ...
        const panel = vscode.window.createWebviewPanel(
            'echo-ai-codebase-stats',
            'Echo AI - Codebase Statistics',
            vscode.ViewColumn.Two,
            { enableScripts: true }
        );
    ...
    const findSymbolCommand = vscode.commands.registerCommand('echo-ai.findSymbol', async () => {
        ...
        if (selected) {
            ...
            vscode.window.showInformationMessage(
                `Found ${selected.symbol.name} at line ${selected.symbol.line} in ${selected.symbol.scope}`
            );
        }
    });
...
function getCodebaseStatsWebviewContent(stats: any, queueStatus: any, performanceMetrics: any): string {
    const languageDistribution = Array.from(stats.languageDistribution.entries() as [string, number][])
