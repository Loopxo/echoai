...
function activate(context) {
    ...
    const generateComplianceReportCommand = vscode.commands.registerCommand('echo-ai.generateComplianceReport', async () => {
        ...
        if (selectedFramework) {
            try {
                ...
                const report = await securityDashboard.generateComplianceReport(selectedFramework);
                // Show report in webview or file
                const action = await vscode.window.showInformationMessage(`${selectedFramework} compliance report generated. Score: ${report.score}/100`, 'View Report', 'Export Report');
    ...
    const fixHardcodedSecretsCommand = vscode.commands.registerCommand('echo-ai.fixHardcodedSecrets', async () => {
        ...
        try {
            ...
            const action = await vscode.window.showInformationMessage(`Found ${secretVulns.length} hardcoded secrets. Fix them automatically?`, 'Fix All', 'Review First');
            if (action === 'Fix All') {
    ...
    const securityEducationCommand = vscode.commands.registerCommand('echo-ai.securityEducation', async () => {
        ...
        if (selectedTopic) {
            try {
                const explanation = await echoProvider.getCompletion(`Provide a comprehensive explanation of "${selectedTopic}" for software developers. Include practical examples and best practices.`, '', 'security', 2000);
                const panel = vscode.window.createWebviewPanel('echo-ai-security-education', `Security Education: ${selectedTopic}`, vscode.ViewColumn.Two, { enableScripts: true });
...
function getSecurityEducationWebviewContent(topic, content) {
    return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
