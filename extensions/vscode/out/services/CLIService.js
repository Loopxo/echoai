...
class CLIService {
    ...
    commandHistory = [];
    historyIndex = -1;
    constructor(context, echoProvider, configManager) {
        this.context = context;
        this.echoProvider = echoProvider;
        this.configManager = configManager;
        this.maxLogs = this.configManager.get('cli.maxLogEntries', 1000);
        this.setupEventListeners();
    }
    static getInstance(context, echoProvider, configManager) {
    ...
    showCLI() {
        ...
        this.panel = vscode.window.createWebviewPanel('echo-ai-cli', 'Echo AI CLI', vscode.ViewColumn.Beside, {
            enableScripts: true,
    ...
    log(level, message, command, context) {
        const entry = {
        ...
        if (this.panel && this.configManager.get('cli.showLogs', true)) {
    ...
    async executeAICommand(command) {
        ...
        try {
            const response = await this.echoProvider.getCompletion(command, context, languageId, 2000);
...
└─ Status: ${this.isProcessing ? 'Processing' : 'Ready'}
    ...
    async listProviders() {
        ...
        const current = this.configManager.get('provider', 'claude');
        const providerList = providers.map(p => p === current ? `● ${p} (current)` : `○ ${p}`).join('\n');
...
Tip: Use /clear to free up log memory
    ...
    getClientConfig() {
        return {
            theme: this.configManager.get('cli.theme', 'dark'),
            showLogs: this.configManager.get('cli.showLogs', true),
            maxLogEntries: this.configManager.get('cli.maxLogEntries', 1000)
        };
    }
    getCLIWebviewContent() {
        const theme = this.configManager.get('cli.theme', 'dark');
        return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
