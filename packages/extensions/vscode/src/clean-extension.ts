...
export async function activate(context: vscode.ExtensionContext) {
    ...
    try {
        ...
        vscode.window.showInformationMessage(
            'EchoAI Ready - Optimized for performance',
            'Open CLI', 'Configure'
        ).then(selection => {
...
function registerCommands(context: vscode.ExtensionContext): void {
    ...
    const memoryCommand = vscode.commands.registerCommand('echoai.memory', async () => {
    ...
    const sessionsCommand = vscode.commands.registerCommand('echoai.sessions', async () => {
        const action = await vscode.window.showQuickPick(
        ...
        if (action === 'Open in CLI') {
            const terminal = vscode.window.createTerminal({ name: 'EchoAI Sessions' });
            terminal.show();
    ...
    const docsCommand = vscode.commands.registerCommand('echoai.docs', async () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
            vscode.window.showInformationMessage('No active editor');
            return;
    ...
    const skillsCommand = vscode.commands.registerCommand('echoai.skills', async () => {
        const skills = [
    ...
    const gatewayCommand = vscode.commands.registerCommand('echoai.gateway', async () => {
        const status = {
            connected: false,
        ...
        vscode.window.showInformationMessage(info, 'Open CLI').then(selection => {
            if (selection === 'Open CLI') {
                const terminal = vscode.window.createTerminal({ name: 'EchoAI Gateway' });
                terminal.show();
