...
There are two types of agents in echoai; primary agents and subagents.
...
echoai comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
echoai comes with one built-in subagent, **General**. We'll look at this below.

---

## Built-in

echoai comes with two built-in primary agents and one built-in subagent.
...
Configure agents in your `echoai.json` config file:
...
- Global: `~/.config/echoai/agent/`
- Per-project: `.echoai/agent/`

```markdown title="~/.config/echoai/agent/review.md"
---
...
If no temperature is specified, echoai uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global echoai config and the project specific config.
...
Configure permissions globally in echoai.json.
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/sst/echoai).
:::
...
```markdown title="~/.config/echoai/agent/docs-writer.md"
---
...
```markdown title="~/.config/echoai/agent/security-auditor.md"
---
