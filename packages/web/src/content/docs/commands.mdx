...
Create `.echoai/command/test.md`:

```md title=".echoai/command/test.md"
---
...
You can add custom commands through the echoai config or by creating markdown files in the `command/` directory.

---

### JSON

Use the `command` option in your echoai [config](/docs/config):

```json title="echoai.jsonc" {4-12}
{
...
- Global: `~/.config/echoai/command/`
- Per-project: `.echoai/command/`

```markdown title="~/.config/echoai/command/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
```md title=".echoai/command/component.md"
---
...
```md title=".echoai/command/analyze-coverage.md"
---
...
```md title=".echoai/command/review-changes.md"
---
...
```md title=".echoai/command/review-component.md"
---
...
echoai includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
