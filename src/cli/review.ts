...
export const reviewCommand = new Command('review')
  ...
  .action(async (files: string[], options) => {
    ...
    const session = await ensureWorkflowSession('Code Review', options.session);
    const task = await kernel.addTask(session.id, {
      kind: 'review',
      title: 'Code review',
      status: 'running',
      detail: 'Scanning changed files for likely regressions',
      metadata: {
    ...
    if (reviewFiles.length === 0) {
      await kernel.updateTask(session.id, task.id, {
        status: 'completed',
        detail: 'No files available for review',
      });
    ...
    if (findings.length === 0) {
      const report = `No review findings.\nScanned ${reviewFiles.length} file(s). Residual risk: logic regressions still require targeted tests and human review.`;
      await persistWorkflowReport(session.id, task.id, 'review-report', 'Review Report', report);
    ...
    const report = formatFindingsReport('review', findings);
    await persistWorkflowReport(session.id, task.id, 'review-report', 'Review Report', report);
...
function formatFindingsReport(
  kind: 'review' | 'security',
  findings: ReviewFinding[]
): string {
  const heading = kind === 'review'
    ? `Found ${findings.length} review finding(s):`
