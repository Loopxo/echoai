...
private let deepLinkLogger = Logger(subsystem: "ai.echoai", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "EchoAI is paused", message: "Unpause EchoAI to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run EchoAI agent?", message: body) else { return }
        }
