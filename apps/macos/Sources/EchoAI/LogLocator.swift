...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["ECHOAI_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        let preferred = URL(fileURLWithPath: "/tmp/echoai")
        return preferred
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("echoai-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("echoai-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/echoai/ (rolling or stdout), or nil if none exist.
