...
private let webChatSwiftLogger = Logger(subsystem: "ai.echoai", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: EchoAIChatTransport, Sendable {
    func requestHistory(sessionKey: String) async throws -> EchoAIChatHistoryPayload {
    ...
    func listSessions(limit: Int?) async throws -> EchoAIChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(EchoAIChatSessionsListResponse.self, from: data)
    }
    ...
    func sendMessage(
        ...
        attachments: [EchoAIChatAttachmentPayload]) async throws -> EchoAIChatSendResponse
    {
    ...
    func events() -> AsyncStream<EchoAIChatTransportEvent> {
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> EchoAIChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                EchoAIGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<EchoAIChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any EchoAIChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = EchoAIChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: EchoAIChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<EchoAIChatView>) -> NSViewController
    {
