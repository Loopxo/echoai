...
enum CommandResolver {
    private static let projectRootDefaultsKey = "echoai.gatewayProjectRootPath"
    private static let helperName = "echoai"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let echoaiEntry = root.appendingPathComponent("echoai.mjs").path
        if FileManager().isReadableFile(atPath: echoaiEntry) { return echoaiEntry }
        let binEntry = root.appendingPathComponent("bin/echoai.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/echoai")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let echoaiPaths = self.echoaiManagedPaths(home: home)
        if !echoaiPaths.isEmpty {
            extras.insert(contentsOf: echoaiPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + echoaiPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func echoaiManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".echoai"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func echoaiExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectEchoAIExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("echoai.mjs").path,
            root.appendingPathComponent("bin/echoai.js").path,
        ]
    ...
    static func hasAnyEchoAIInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.echoaiExecutable(searchPaths: searchPaths) != nil { return true }
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let echoaiPath = self.projectEchoAIExecutable(projectRoot: root) {
                return [echoaiPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "echoai", subcommand] + extraArgs
            }
            if let echoaiPath = self.echoaiExecutable(searchPaths: searchPaths) {
                return [echoaiPath, subcommand] + extraArgs
            }

            let missingEntry = """
            echoai entrypoint missing (looked for dist/index.js or echoai.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real echoai CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/echoai"
        ...
        \(cliSection)
        \(projectSection)
        if command -v echoai >/dev/null 2>&1; then
          CLI="$(command -v echoai)"
          echoai \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/echoai.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/echoai.mjs"
            node "$PRJ/echoai.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/echoai.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/echoai.js"
            node "$PRJ/bin/echoai.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent echoai"
          pnpm --silent echoai \(quotedArgs);
        else
          echo "echoai CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? EchoAIConfigFile.loadDict()
