...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (EchoAI token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: EchoAIOAuthStore.AnthropicOAuthStatus = EchoAIOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "ai.echoai", category: "anthropic-oauth")
...
enum EchoAIOAuthStore {
    static let oauthFilename = "oauth.json"
    private static let providerKey = "anthropic"
    private static let echoaiOAuthDirEnv = "ECHOAI_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "EchoAI OAuth token file not found"
            case .unreadableFile: "EchoAI OAuth token file not readable"
            case .invalidJSON: "EchoAI OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in EchoAI OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "EchoAI OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.echoaiOAuthDirEnv]?
        ...
        let preferred = home.appendingPathComponent(".echoai", isDirectory: true)
