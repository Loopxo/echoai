...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.echoai/echoai.json. Prefer ECHOAI_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.echoai/echoai.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.echoai/echoai.json. Restart the gateway to apply."
        }
