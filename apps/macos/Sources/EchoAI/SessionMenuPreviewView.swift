...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.echoai", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> EchoAISessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: EchoAISessionPreviewEntry,
    ...
    private static func cache(payload: EchoAISessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: EchoAIChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [EchoAIKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [EchoAIKit.AnyCodable]) -> [EchoAIChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(EchoAIChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: EchoAIChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: EchoAIChatMessage) -> Bool {
    ...
    private static func toolNames(for message: EchoAIChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: EchoAIChatMessage) -> String? {
