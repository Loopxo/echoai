...
enum EchoAILogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = EchoAIOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = EchoAIFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        EchoAILogging.bootstrapIfNeeded()
        let label = EchoAILogging.makeLabel(subsystem: subsystem, category: category)
...
struct EchoAIOSLogHandler: LogHandler {
...
struct EchoAIFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = EchoAILogging.parseLabel(self.label)
