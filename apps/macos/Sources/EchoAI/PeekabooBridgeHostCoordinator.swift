...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.echoai", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: EchoAIPeekabooBridgeServices?
    private static var echoaiSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("EchoAI", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = EchoAIPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.echoaiSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.echoaiSocketPath, privacy: .public)")
    }
...
@MainActor
private final class EchoAIPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.echoai.peekaboo")
