...
enum DebugActions {
    private static let verboseDefaultsKey = "echoai.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "echoai.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = EchoAIPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "EchoAI", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = EchoAIPaths.configURL
        guard
