...
extension OnboardingView {
    ...
    func refreshLocalGatewayProbe() async {
        ...
        await MainActor.run {
            ...
            let expectedTokens = ["node", "echoai", "tsx", "pnpm", "bun"]
    ...
    func refreshAnthropicOAuthStatus() {
        _ = EchoAIOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        let previous = self.anthropicAuthDetectedStatus
        let status = EchoAIOAuthStore.anthropicOAuthStatus()
    ...
    func verifyAnthropicOAuthIfNeeded(force: Bool = false) async {
        ...
        guard let refresh = EchoAIOAuthStore.loadAnthropicOAuthRefreshToken(), !refresh.isEmpty else {
        ...
        do {
            ...
            try EchoAIOAuthStore.saveAnthropicOAuth(updated)
            self.refreshAnthropicOAuthStatus()
