...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: EchoAIOAuthStore.AnthropicOAuthStatus = EchoAIOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: EchoAIOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = EchoAIOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = EchoAIOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try EchoAIOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. EchoAI can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: EchoAIOAuthStore.AnthropicOAuthStatus,
