...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: EchoAIProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.echoai", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: EchoAIKit.AnyCodable]()) {
                $0[$1.key] = EchoAIKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: EchoAIProtocol.AnyCodable?) -> [String: EchoAIProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: EchoAIProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: EchoAIKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: EchoAIProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("echoai.control.heartbeat")
    static let controlAgentEvent = Notification.Name("echoai.control.agent")
}
