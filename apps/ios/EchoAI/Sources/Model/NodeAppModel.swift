...
final class NodeAppModel {
    ...
    private func handleCanvasA2UIAction(body: [String: Any]) async {
        ...
        guard let name = EchoAICanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId: String = {
        ...
        let contextJSON = EchoAICanvasA2UIAction.compactJSON(userAction["context"])
        let sessionKey = self.mainSessionKey

        let messageContext = EchoAICanvasA2UIAction.AgentMessageContext(
            actionName: name,
            session: .init(key: sessionKey, surfaceId: surfaceId),
            component: .init(id: sourceComponentId, host: host, instanceId: instanceId),
            contextJSON: contextJSON)
        let message = EchoAICanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        let js = EchoAICanvasA2UIAction.jsDispatchA2UIActionStatus(actionId: actionId, ok: ok, error: errorText)
        do {
    ...
    private func resolveA2UIHostURL() async -> String? {
        ...
        return base.appendingPathComponent("__echoai__/a2ui/").absoluteString + "?platform=ios"
    }
    ...
    func requestLocationPermissions(mode: EchoAILocationMode) async -> Bool {
    ...
    private func sendAgentRequest(link: AgentDeepLink) async throws {
        ...
        // (Key-based unattended auth is handled on macOS for echoai:// links.)
    ...
    private func handleInvoke(_ req: BridgeInvokeRequest) async -> BridgeInvokeResponse {
        ...
        do {
            switch command {
            case EchoAILocationCommand.get.rawValue:
                return try await self.handleLocationInvoke(req)
            case EchoAICanvasCommand.present.rawValue,
                 EchoAICanvasCommand.hide.rawValue,
                 EchoAICanvasCommand.navigate.rawValue,
                 EchoAICanvasCommand.evalJS.rawValue,
                 EchoAICanvasCommand.snapshot.rawValue:
                return try await self.handleCanvasInvoke(req)
            case EchoAICanvasA2UICommand.reset.rawValue,
                 EchoAICanvasA2UICommand.push.rawValue,
                 EchoAICanvasA2UICommand.pushJSONL.rawValue:
                return try await self.handleCanvasA2UIInvoke(req)
            case EchoAICameraCommand.list.rawValue,
                 EchoAICameraCommand.snap.rawValue,
                 EchoAICameraCommand.clip.rawValue:
                return try await self.handleCameraInvoke(req)
            case EchoAIScreenCommand.record.rawValue:
                return try await self.handleScreenRecordInvoke(req)
            default:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: EchoAINodeError(code: .invalidRequest, message: "INVALID_REQUEST: unknown command"))
            }
        } catch {
            ...
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: EchoAINodeError(code: .unavailable, message: error.localizedDescription))
        }
    }
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(EchoAILocationGetParams.self, from: req.paramsJSON)) ??
            EchoAILocationGetParams()
        ...
        let payload = EchoAILocationPayload(
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case EchoAICanvasCommand.present.rawValue:
            let params = (try? Self.decodeParams(EchoAICanvasPresentParams.self, from: req.paramsJSON)) ??
                EchoAICanvasPresentParams()
        ...
        case EchoAICanvasCommand.hide.rawValue:
            return BridgeInvokeResponse(id: req.id, ok: true)
        case EchoAICanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(EchoAICanvasNavigateParams.self, from: req.paramsJSON)
            self.screen.navigate(to: params.url)
            return BridgeInvokeResponse(id: req.id, ok: true)
        case EchoAICanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(EchoAICanvasEvalParams.self, from: req.paramsJSON)
            let result = try await self.screen.eval(javaScript: params.javaScript)
            let payload = try Self.encodePayload(["result": result])
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case EchoAICanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(EchoAICanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleCanvasA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case EchoAICanvasA2UICommand.reset.rawValue:
            ...
            (() => {
              ...
              if (!host) return JSON.stringify({ ok: false, error: "missing echoaiA2UI" });
              return JSON.stringify(host.reset());
            })()
            """)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case EchoAICanvasA2UICommand.push.rawValue, EchoAICanvasA2UICommand.pushJSONL.rawValue:
            let messages: [AnyCodable]
            if command == EchoAICanvasA2UICommand.pushJSONL.rawValue {
                let params = try Self.decodeParams(EchoAICanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
                messages = try EchoAICanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
            } else {
                do {
                    let params = try Self.decodeParams(EchoAICanvasA2UIPushParams.self, from: req.paramsJSON)
            ...
            let messagesJSON = try EchoAICanvasA2UIJSONL.encodeMessagesJSONArray(messages)
            let js = """
            (() => {
              try {
                const host = globalThis.echoaiA2UI;
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case EchoAICameraCommand.list.rawValue:
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case EchoAICameraCommand.snap.rawValue:
            self.showCameraHUD(text: "Taking photo…", kind: .photo)
            self.triggerCameraFlash()
            let params = (try? Self.decodeParams(EchoAICameraSnapParams.self, from: req.paramsJSON)) ??
                EchoAICameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case EchoAICameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(EchoAICameraClipParams.self, from: req.paramsJSON)) ??
                EchoAICameraClipParams()
    ...
    private func handleScreenRecordInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(EchoAIScreenRecordParams.self, from: req.paramsJSON)) ??
            EchoAIScreenRecordParams()
...
private extension NodeAppModel {
    func locationMode() -> EchoAILocationMode {
        let raw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        return EchoAILocationMode(rawValue: raw) ?? .off
    }
