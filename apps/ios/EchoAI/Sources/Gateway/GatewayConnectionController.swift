...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [EchoAICapability.canvas.rawValue, EchoAICapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(EchoAICapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(EchoAICapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = EchoAILocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(EchoAICapability.location.rawValue) }
    ...
    private func currentCommands() -> [String] {
        ...
        if caps.contains(EchoAICapability.camera.rawValue) {
        ...
        if caps.contains(EchoAICapability.location.rawValue) {
