...
class NodeForegroundService : Service() {
  ...
  override fun onCreate() {
    super.onCreate()
    ensureChannel()
    val initial = buildNotification(title = "EchoAI Node", text = "Starting…")
    startForegroundWithTypes(notification = initial, requiresMic = false)

    val runtime = (application as EchoAIApp).runtime
    notificationJob =
      scope.launch {
        combine(
          runtime.statusText,
          ...
          val title = if (connected) "EchoAI Node · Connected" else "EchoAI Node"
          val voiceSuffix =
  ...
  override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
    when (intent?.action) {
      ACTION_STOP -> {
        (application as EchoAIApp).runtime.disconnect()
        stopSelf()
  ...
  private fun ensureChannel() {
    ...
    val channel =
      NotificationChannel(
        ...
        description = "EchoAI node connection status"
        setShowBadge(false)
      }
  ...
  companion object {
    ...
    private const val ACTION_STOP = "ai.echoai.android.action.STOP"
